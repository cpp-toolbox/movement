 ...
 */
glm::vec3 god_mode_delta_pos(glm::vec3 forward_vector,
                             GodModeInput god_mode_input, float delta_time,
                             float base_move_speed = 2);

glm::vec3 get_new_god_mode_velocity(glm::vec3 velocity, glm::vec3 forward,
                                    GodModeInput god_mode_input,
                                    float delta_time,
...
constexpr float MOVING_FRICTION_PER_SECOND =
    0.5; // after one second your velocity will be halved
constexpr float STOPPING_FRICTION_PER_SECOND = 0.01;

template <FPSMovementInputLike Input>
glm::vec3 get_new_fps_character_velocity(glm::vec3 current_velocity,
                                         const Input &input_state,
                                         glm::vec3 xy_forward_vector_camera,
                                         double dt, GroundState ground_state) {
  LogSection _(*global_logger, "get_new_fps_character_velocity", false);

  global_logger->info("=== Start velocity update ===");
  global_logger->info("Current velocity: x={}, y={}, z={}", current_velocity.x,
  ...
  glm::vec3 forward = glm::normalize(
      glm::vec3(xy_forward_vector_camera.x, 0.0f, xy_forward_vector_camera.z));
  glm::vec3 right = glm::normalize(glm::cross(forward, glm::vec3(0, 1, 0)));
  global_logger->info("Forward vector: x={}, y={}, z={}", forward.x, forward.y,
                      forward.z);
  global_logger->info("Right vector: x={}, y={}, z={}", right.x, right.y,
                      right.z);

  glm::vec3 look_input_vec = input_vec.x * right + input_vec.y * forward;
  global_logger->info("Look input vector (world-space): x={}, y={}, z={}",
                      look_input_vec.x, look_input_vec.y, look_input_vec.z);

  // Horizontal velocity only
  glm::vec3 horizontal_velocity =
      glm::vec3(current_velocity.x, 0.0f, current_velocity.z);
  global_logger->info("Initial horizontal velocity: x={}, y={}, z={}",
                      horizontal_velocity.x, horizontal_velocity.y,
                      horizontal_velocity.z);

  if (glm::length(look_input_vec) > 0.0f) {
    // Accelerate in the input direction
    horizontal_velocity +=
        look_input_vec * static_cast<float>(ACCELERATION * dt);
    global_logger->info("Accelerated horizontal velocity: x={}, y={}, z={}",
                        horizontal_velocity.x, horizontal_velocity.y,
                        horizontal_velocity.z);

    // Apply moving friction
    horizontal_velocity *= pow(MOVING_FRICTION_PER_SECOND, dt);
    global_logger->info("After moving friction: x={}, y={}, z={}",
                        horizontal_velocity.x, horizontal_velocity.y,
                        horizontal_velocity.z);
  } else {
    ...
    global_logger->info(
        "No input detected. After stopping friction: x={}, y={}, z={}",
        horizontal_velocity.x, horizontal_velocity.y, horizontal_velocity.z);
  }
  ...
  global_logger->info(
      "Updated current_velocity (horizontal only): x={}, y={}, z={}",
  ...
  if (ground_state == GroundState::OnGround) {
    ...
    if (input_state.jump_pressed) {
      ...
      global_logger->info("Jump pressed. New vertical velocity: y={}",
                          current_velocity.y);
    }
  }
...
public:
  RotatingMovement(double radius, const glm::vec3 &origin = glm::vec3(0.0f),
                   const glm::vec3 &rotation_axis = glm_utils::y,
                   double rotation_rate_hz = 1.0)
      : radius_(radius), rotation_axis_(glm::normalize(rotation_axis)),
        rotation_rate_hz_(rotation_rate_hz), origin_(origin),
  ...
  glm::vec3 get_current_position(double dt) {
    ...
    double angular_velocity =
        rotation_rate_hz_ * 2.0 * std::numbers::pi; // radians per sec
